...
static void ota_example_task(void *pvParameter)
{
    ...
    ESP_LOGI(TAG, "Starting OTA example updated... @ %p flash %s", ota_example_task, CONFIG_ESPTOOLPY_FLASHSIZE);
    ...
    {
        ESP_LOGW(TAGO, "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x",
                 configured->address, running->address);
        ESP_LOGW(TAGO, "(This can happen if either the OTA boot data or preferred boot image become corrupted somehow.)");
    }
    ESP_LOGI(TAGO, "Running partition type %d subtype %d (offset 0x%08x)",
             running->type, running->subtype, running->address);

    /* Wait for the callback to set the CONNECTED_BIT in the
       event group.
        */
    xEventGroupWaitBits(upgrade_event_group, CONNECTED_BIT,
                        false, true, portMAX_DELAY);
    ESP_LOGI(TAGO, "Start to Connect to Server....");

    /*connect to http server*/
    if (connect_to_http_server())
    {
        ESP_LOGI(TAGO, "Connected to http server");
    }
    else
    {
        ESP_LOGE(TAGO, "Connect to http server failed!");
        task_fatal_error();
    }
    ...
    get_len = asprintf(&http_request, GET_FORMAT, EXAMPLE_FILENAME, EXAMPLE_SERVER_IP, EXAMPLE_SERVER_PORT);
    if (get_len < 0)
    {
        ESP_LOGE(TAGO, "Failed to allocate memory for GET request buffer");
        task_fatal_error();
    }
    ...
    {
        ESP_LOGI(TAGO, "Send GET request to server succeeded");
    }

    update_partition = esp_ota_get_next_update_partition(NULL);
    ESP_LOGI(TAGO, "Writing to partition subtype %d at offset 0x%x",
    ...
    ESP_LOGI(TAGO, "esp_ota_begin succeeded");
    ...
    while (flag)
    {
        ...
        if (buff_len < 0)
        { /*receive error*/
            ESP_LOGE(TAGO, "Error: receive data error! errno=%d", errno);
            task_fatal_error();
        }
        else if (buff_len > 0)
        { /*deal with response body*/
            ...
            ESP_LOGI(TAGO, "Have written image length %d", binary_file_length);
        }
        else if (buff_len == 0)
        { /*packet over*/
            flag = false;
            ESP_LOGI(TAGO, "Connection closed, all packets received");
            close(socket_id);
        }
        else
        {
            ESP_LOGE(TAGO, "Unexpected recv result");
        }
    ...
    ESP_LOGI(TAGO, "Total Write binary data length : %d", binary_file_length);
    ...
    ESP_LOGI(TAGO, "Prepare to restart system!");

    connect_to_http_server();
    get_len = asprintf(&http_request, GET_FORMAT, "Upgrade OK! Prepare to restart system!", EXAMPLE_SERVER_IP, EXAMPLE_SERVER_PORT);
    res = send(socket_id, http_request, get_len, 0);
    free(http_request);
    http_request = NULL;
    free(http_request);
