...
void softuart_print_rx(uint8_t uart_no) {
    printf("RRRR buffer.receive_buffer: /n");

    softuart_t *uart = uarts + uart_no;

    for (size_t i = 0; i < SOFTUART_MAX_RX_BUFF; i++)
    {
        printf("buffer.receive_buffer: %x /n",uart->buffer.receive_buffer[i]);
    }
